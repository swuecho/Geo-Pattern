...
my $FILL_COLOR_DARK  = "#222";
my $FILL_COLOR_LIGHT = "#ddd";
my $STROKE_COLOR     = "#000"
my $STROKE_OPACITY   = 0.02;
my $OPACITY_MIN      = 0.02;
my $OPACITY_MAX      = 0.15;

has 'string' => ( is => 'ro', isa => 'Str');
has 'hash' => ( is => 'ro', isa => 'Str', lazy_build => 1 );
sub _build_hash ($self)  { Digest::SHA1::sha1_hex($self->string) };

has 'opts' => ( is => 'ro', isa => 'HashRef', lazy_build => 1 );

sub _build_opts($self)  { return {%$DEFAULT, %$opts} };

has 'svg' => ( is => 'ro', isa => 'Geo::Pattern::SVG', default => sub { Geo::Pattern::SVG->new );;
#has 'svg_string' => ( is => 'ro', isa => 'Str', lazy_build => 1 );
#sub _build_svg_string($self)  { return $self->svg->to_s };
...
sub base64_string($self) {
      encode_base64($self->to_s)
}

sub uri_image($self) {
       my $base64_string = shift->base64_string;
      "url(data:image/svg+xml;base64,$base64_string);"
}

sub generate_background($self) {
	my ($r,$g, $b);
	if (my $color_name = $self->opts->{color}) {
            ($r, $g, $b) = Convert::Color::X11->new($color_name)->rgb8;
	} else {
            #TODO
	}
      $self->svg->rect(0, 0, "100%", "100%", {"fill" => "rgb($r, $g, $b)"})
